...
from home7.pages.AdminPage import AdminPage
from home7.pages.RegisterPage import RegisterPage
from home7.pages.elements.AddProductForm import AddProductForm
...
def test_change_currency(browser):
    page_path = "http://192.168.0.4:8081/"
    with allure.step(f"Открываем страницу {page_path}"):
...
def test_add_product(browser, product, price, quantity, module):
    page_path = "http://192.168.0.4:8081/admin"
    login = "user"
    password = "bitnami"
    with allure.step(f"Открываем страницу {page_path}"):
        browser.get(page_path)
