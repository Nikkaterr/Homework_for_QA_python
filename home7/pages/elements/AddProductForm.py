...
from home7.pages.BasePage import BasePage
...
class AddProductForm(BasePage):
    ...
    def _input_product_name(self, name):
        self.logger.info(f"Input product name: {name}")
    ...
    def _input_meta_tag_title(self, title):
        self.logger.info(f"Input meta tag: {title}")
    ...
    def _input_model(self, model):
        self.logger.info(f"Input model: {model}")
    ...
    def _input_quantity(self, quantity):
        ...
        self.logger.info(f"Input price: {quantity}")
        element.send_keys(Keys.BACK_SPACE)
